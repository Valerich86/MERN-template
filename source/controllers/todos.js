...
import { getList, getItem, addItem, setDoneItem, deleteItem, getMostActiveUsers, 
    deleteAll, getListWithoutFilter } from "../models/todos.js";
...
export function addPage(req, res){
    res.render('add', {title: 'Добавление записи'});
}
...
export async function removeAll(req, res, next){
    const list = await getListWithoutFilter(req.user.id);
    if (list){
